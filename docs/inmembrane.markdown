...
In modern scripting languages like Python, such procedures can be automated and libraries have been created to automatically query a web-site, and extract the results from a web-page. As an example of the ease of automating a pipeline that includes querying a web-site, we provide two modules for predicting beta-barrel proteins, TBMETA and OMPF. These are shown in the plugins in the file blah blah. Specifically, we use the Twill library for setting up the query and uploading our sequences onto the website. And we use Beautiful Soup to parse the web-page and put the results in the `protein` data structure. The beauty of this approach is that we can code this consistently with our API of creating a function `run_tmbeta(params, protein)`.
