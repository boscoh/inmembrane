...
Since the core algorithm in _SurfG+_ is relatively straightforward, we decided to replicate and expand upon the functionality of _SurfG+_ by writing _inmembrane_ in a modern scripting language. This lead to considerable simplifiction and clarification of the code base. Compared with the _SurfG+_ which is has 5,731 lines of source code (SLOC) (svn revision 48, SLOCCount v2.26), primariliy in Java, _inmembrane_, without dependencies, is around ~1900 SLOC of Python code and includes additional functionality not offered by _SurfG+_. The smaller code base is substantially easier to reuse and repurpose for other users. Here, we discuss the issues involved in writing robust and accessible bioinformatic source code.
