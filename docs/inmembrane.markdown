...
Since the core algorithm in _SurfG+_ is relatively straightforward, we decided to replicate and expand upon the functionality of _SurfG+_ by writing _inmembrane_ in a modern scripting language. This lead to considerable simplifiction and clarification of the code base. Compared with the _SurfG+_ Java source code of 700K, _inmembrane_, without dependencies, is around 70K of Python code and includes additional functionality not offered by _SurfG+_. In particular, we have provided an extensive web-based API that dramatically simplifies the usage of _inmembrane_. The smaller code base is substantially easier to reuse and repurpose for other users. Here, we discuss the issues involved in writing robust and accessible bioinformatic source code.
...
The use of web services constitutes a usability trade-off against the use of local external binaries. Web services assume that the internet is readily available, that the service provider has ensured good up-time, and that the web-service protocol will not be deprecated. Whilst for well-supported services, this is a reasonable assumption, for smaller services, this may not. In contrast, installing local binaries requires a much greater effort in installing for a new user. Each external binary has to be installed, tested, debugged and made consistent with _inmembrane_. Another limitation is platform specificty: in _inmembrane_ for instance, the full set of external binaries is only available on the Linux platform. Nevertheless, the advantage in such an approach is that, once installed, the user can be confident in the future operation of _inmembrane_.

This raises one other point about durability. The purpose of _inmembrane_ is to stitch together various existing binaries in way that is transparent for the user. Instead of writing a monolithic workflow, it is a light framework that mixes the use of web-scraping API's and the use of local binaries. Despite the fact that _inmembrane_ is open-source with a certain measure of guarantee against future loss, the dependencies of _inmembrane_ are not. As such, there is no guarantee that these programs will exist in the future. It is hoped that more bioinformatic software will move towards an open-source, publicly hosted resource, which will give more of a guarantee. The choice for _inmembrane_ is designed to push that goal along.
